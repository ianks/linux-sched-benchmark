...
int get_scheduler_pref(int argc, char* pref)
{
  int policy;

  if(argc > 2){
    if(!strcmp(pref, "SCHED_OTHER"))
      policy = SCHED_OTHER;
    else if(!strcmp(pref, "SCHED_FIFO"))
      policy = SCHED_FIFO;
    else if(!strcmp(pref, "SCHED_RR"))
      policy = SCHED_RR;
    else{
...
int set_scheduler_policy(int argc, char* argv[])
{
  int policy;
  policy =  get_scheduler_pref(argc, argv[5]);
...
int cleanup_files(char* output_file)
{
  ...
  if(status == 0)
    ...
    printf("Unable to cleanup the files\n");
    perror("Error");
  }
...
int main(int argc, char* argv[]){
  ...
  set_scheduler_policy(argc, argv);
